...
class PHPImageEditor {
    ...
    /**
     * Floxim edit, text by consts 
     */
    function LoadLanguage () {
        $all_constatnts = get_defined_constants();
        if ( $all_constatnts ) {
            foreach ( $all_constatnts as $name => $value ) {
                if (substr($name, 0,4) == 'PIE_' ) {
                    $this->texts[ substr($name, 4) ] = $value; 
                }
            }
        }
        //$this->texts
