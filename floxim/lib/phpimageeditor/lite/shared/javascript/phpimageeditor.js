...
// floxim edit
function pie_ajax_post_save ( callback ) {
    if (validate_form()) {
...
function ajax_post( callback )
{
    if ($('#loading').css('display') == 'none') 
    {
        ...
        $('.pie_button_update').attr('disabled','disabled');
        $('#btnsave').attr('disabled','disabled');
        $('.pie_button_undo').attr('disabled','disabled');
        $('#brightness_slider_track').slider('disable');
        $('#contrast_slider_track').slider('disable');
        $('#loading').css('display','block');
        $('#loading_bar').width(0);
        $('#loading_bar').animate({
            width: document.getElementById('loading').offsetWidth-30
            }, document.getElementById('loading').offsetWidth*10);
...
function reload_mouse_crop()
{
    objCrop.destroy();
    objCrop = $.Jcrop('#image',{
        onChange: set_crop_values,
        onSelect: set_crop_values, 
        aspectRatio: $("input#cropkeepproportions").attr('checked') ? $("#cropkeepproportionsratio").val() : 0
        });		
...
function activate_form()
{
    ...
    $('.pie_button_update').removeAttr('disabled');
    $('#btnsave').removeAttr('disabled');
    $('.pie_button_undo').removeAttr('disabled');
...
/**
 * Floxim edit: убрал использование классов selected в пользу jquery ui
 */
    ...
    for (i = 0; i < 4; i++)
    {
        if ($('#panel_'+i) != null)
        {
            ...
            if (i == selectedIndex)
            {
                ...
                $("#menuitem_"+i).closest('li').addClass("ui-tabs-selected ui-state-active");
            ...
            else
            {
                ...
                $("#menuitem_"+i).closest('li').removeClass("ui-tabs-selected ui-state-active");				
...
function phpimageeditor_init()
{
    objCrop = $.Jcrop('#image',{
        onChange: set_crop_values,
        onSelect: set_crop_values, 
        aspectRatio: $("input#cropkeepproportions").attr('checked') ? $("input#cropkeepproportionsratio").val() : 0
        });
    ...
    $("#contrast_slider_track").slider({
        value: parseInt($("#contrast").val()) + ContrastMax, 
        min: 1, 
        max: ((ContrastMax*2)+1), 
        step: 1,
        stop: function(event,ui)
    ...
    $("#brightness_slider_track").slider({
        value: (parseInt($("input#brightness").val())+BrightnessMax),
        min: 1,
        max: ((BrightnessMax*2)+1),
        step: 1,
        stop: function(event,ui)
        {        
            if (validate_form())
    ...
    $(".pie_button_update").click(function()
    {
        if (validate_form())
        {
    ...
    $(".pie_button_undo").click(function()
    {
        if (validate_form())
        {
    ...
    {
        focus_on_enter($("input.pie_button_update"), event);
    });
    ...
    $('#cropkeepproportions').change(function(e) 
    {
        if (objCrop != null)
        {
            ...
            {
                objCrop.setOptions({
                    aspectRatio: $("#cropkeepproportionsratio").val()
                });
            }   
            else
            {   
