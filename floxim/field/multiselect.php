...
class fx_field_multiselect extends fx_field_select {
    ...
    public function content_procces(fx_content $content, fx_infoblock_content $infoblock = null, $hash_obj = null) {
         ...
         * в режиме редактирования вывод $f_name должен обрамиться тегом, но и $f_name->method() должен работать 
         */
        $result = array();
    ...
    public function format_settings() {
        ...
        $fields[] = array('name' => 'format[show]', 'label' => 'Показывать как', 'values' => $show, 'type' => 'select', 'value' => $this->_show_type);
        return $fields;
    }
