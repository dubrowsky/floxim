...
class fx_thumb {
	...
	public function __construct($source_http_path, $config) {
      ...
      $source_path = realpath($source_path);
		...
		if (!isset(self::$_types[$info['imagetype']])) {
			// íåïðàâèëüíûé/íåèçâåñòíûé òèï êàðòèíêè
	...
	protected function _calculateSize($params, $source = null) {
		...
		// äëÿ íà÷àëà: width:200 => min-width:200, max-width:200
		if ($w) {
			if (!$miw) {
				$miw = $w;
			}
		...
		// çàäàëè òîëüêî øèðèíó
		if ($w && !$h) {
			$h = $w * 1/$ratio;
		} 
		// çàäàëè òîëüêî âûñîòó
		elseif ($h && !$w) {
			$w = $h * $ratio;
		} 
		// íå çàäàëè íè÷åãî
		...
		*/
		...
		// à òåïåðü ñ÷èòàåì ïî òàáëè÷êå http://www.w3.org/TR/CSS21/visudet.html#min-max-widths
...
*/
		...
		// ñ øèðèíîé îê. ïðîáëåìû òîëüêî ñ âûñîòîé
		// âûøå
		elseif ($h > $mah) {
			// #3
			$w = max($mah * $w/$h, $miw);
		...
		return array('width' => round($w), 'height' => round($h));
	}
	...
	public function Resize($params = null) {
		...
		// âû÷èñëÿåì ðàçìåðû èñõîäÿ èç min-max, ðàçìåðîâ êàðòèíêè è çàäàíûõ w-h
		...
		// îïðåäåëÿåì êîýô. ìàñøòàáèðîâàíèÿ
		$scale = 1;
		// è îòñòóïû äëÿ îáðåçàíèÿ
		$crop_x = 0;
		$crop_y = 0;
		// è øèðèíó-âûñîòó âûðåçàåìîãî êóñêà
		$crop_width = $width;
		$crop_height = $height;
		...
		if (isset($st['crop']) && $st['crop']) {
			...
			if (isset($st['crop_offset']) && in_array($st['crop_offset'], array('top', 'middle', 'bottom'))) {
