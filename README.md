...
2. Выполняем дамп из файла /install/sql/core.sql
3. Переименовывваем config.sample.php в config.php и добавляем туда свой конфиг подключения к БД
4. Настраиваем апач, чтобы папка с проектом откликалась по хостам "floxim.loc" и "playground.fx" (наши демо-сайты)
5. Входим под админом вот так: http://floxim.loc/floxim/ | dubr.cola@gmail.com | 123
...
Есть небольшая утилитка для отладки - дампит любые переменные в лог. Использовать так:
        ...
        dev_log($var1, $var2, 'ololo');

После этого смотрим результаты по адресу http://floxim/_devlog/show.php - видим список лог-файлов, метод, URL и время создания. Щелкаем нужный - видим результат. Объекты-массивы сворачиваются для удобства восприятия. Любые переменные можно вывести на экран в таком же виде прямо из скрипта, написав:

       echo fx_debug($var1, $var2);
       
Запись в лог удобно использовать, чтобы не ломать сайт, а вывод через fx_debug - в маленьких скриптах-тестах. 
